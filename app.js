...
(() => {
  ...
  let L = 100;                 // rope length (computed below)
  const G = 2500;              // gravity (px/s^2)
  const DAMP = 0.995;          // air drag
  const MAX_DEG = 70;          // hard limit
  const STR_STRETCH = 0.06;    // string stretch proportional to speed

  // Measure effective length (pivot -> card center)
  const measure = () => {
  ...
  const onMove = (e) => {
    ...
    const prev = theta;
    setFromPointer(p.clientX, p.clientY);
    omega = (theta - prev) / dt; // momentum on release
    lastMoveT = tNow;
  };
